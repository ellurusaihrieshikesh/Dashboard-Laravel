...
class DailySaleController extends Controller
{
    ...
    {
        $user = auth()->user();
        $sales = DailySale::where('user_id', $user->id)
            ->orderBy('sale_date', 'desc')
            ->paginate(10);

        // Get daily sales data for the past week
        $dailySalesData = DailySale::where('user_id', $user->id)
            ->select(
                'sale_date',
        ...
        $topProducts = DailySale::where('user_id', $user->id)
            ->select(
                'product_name',
        ...
        $productDistribution = DailySale::where('user_id', $user->id)
            ->select(
                'product_name',
        ...
        // Summary cards
        $monthlyTotalSales = DailySale::where('user_id', $user->id)
            ->whereMonth('sale_date', now()->month)
            ->sum('amount');
        
        $topProductName = $topProducts->first()->product_name ?? 'N/A';
        
        $todaysTotalSales = DailySale::where('user_id', $user->id)
            ->whereDate('sale_date', now())
            ->sum('amount');
        
        $todaysOrderCount = DailySale::where('user_id', $user->id)
            ->whereDate('sale_date', now())
            ->count();
        
        $averageSaleAmount = DailySale::where('user_id', $user->id)
            ->avg('amount');
    ...
    public function todaySummary()
    {
        $user = auth()->user();
        $today = Carbon::today();
        $totalSales = DailySale::where('user_id', $user->id)
            ->whereDate('sale_date', $today)
            ->sum('amount');
        
        $salesCount = DailySale::where('user_id', $user->id)
            ->whereDate('sale_date', $today)
            ->count();
    ...
    {
        $user = auth()->user();
        $sale = DailySale::where('user_id', $user->id)
            ->findOrFail($id);
        $sale->delete();
    ...
    public function generatePdf()
    {
        ...
        $productSales = DailySale::where('user_id', $user->id)
            ->select(
                'product_name',
                DB::raw('COUNT(*) as count')
            )
    ...
    private function generateLineChart($labels, $data)
    {
        ...
        foreach ($labels as $index => $label) {
            ...
            $html .= '<td style="padding: 5px; border: 1px solid #ddd;">Rs. ' . number_format($value, 2) . '</td>';
            $html .= '<td style="padding: 5px; border: 1px solid #ddd;">';
            $html .= '<div style="background-color: #3B82F6; height: 2px; width: 100%; position: relative;">';
            $html .= '<div style="position: absolute; top: -4px; left: ' . ($value / $maxValue * 100) . '%; width: 8px; height: 8px; background-color: #3B82F6; border-radius: 50%;"></div>';
            $html .= '</div>';
            $html .= '</td>';
            $html .= '</tr>';
        }
